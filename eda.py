...
def run_eda() :
    ...
    choice_column = st.selectbox('컬럼을 선택하기', column_list)
    ...
    st.text('컬럼들을 2개이상 선택하시면, 컬럼들의 상관계수를 보여줍니다.')

    corr_colunm_list = ['Age', 'Annual Salary', 'Credit Card Debt', 'Net Worth', 'Car Purchase Amount']
    selected_columns = st.multiselect('컬럼을 선택하세요.', corr_colunm_list)

    # 2개 이상 선택했을때와 그렇지 않을때로 나눠서 개발
